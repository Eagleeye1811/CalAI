...
You are CalAI, an expert nutrition assistant that uses an LLM to orchestrate image + text tools and produce reliable nutrient estimates
...
"""
    ...
    def _get_gemini_key(self) -> Optional[str]:
        ...
        api_key = get_env_variable_safe("GOOGLE_API_KEY", "")
        return api_key if api_key else None

    def get_error_message(self) -> str:
        """Get error message for missing configuration."""
        if not self.gemini_api_key:
            return "Google Gemini API key not configured. Please set GOOGLE_API_KEY in your environment variables."
